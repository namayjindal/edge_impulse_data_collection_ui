...
from PyQt5.QtWidgets import (QApplication, QWizard, QWizardPage, QLabel,
                             QLineEdit, QVBoxLayout, QDateEdit, QPushButton,
...
async def connect_sensors():
    tasks = []
    devices = await scan_for_sensors()
...
class AsyncRunner(QThread):
    ...
    def run(self):
        loop = asyncio.new_event_loop()
        asyncio.set_event_loop(loop)
        loop.run_until_complete(self.scan_for_sensors())
        loop.close()
        self.updateStatus.emit("Sensors scanned.")  # update status when scanning is done
    ...
    async def connect_to_sensors(self):
        await connect_sensors()
        self.updateStatus.emit("Connected to sensors.")  # update status when connections are done

    def start_tracking(self):
        loop = asyncio.new_event_loop()
        asyncio.set_event_loop(loop)
        loop.run_until_complete(self.connect_to_sensors())  # run the connection and tracking task
        loop.close()
...
class MainPage(QWizardPage):
    ...
    def initUI(self):
        ...
        # Buttons
        self.connect_button = QPushButton('Connect to Sensors', self)
        self.connect_button.clicked.connect(self.connectSensors)
        ...
        self.async_connector = AsyncConnector()  # new object for connection only
        self.async_connector.updateStatus.connect(self.setStatus)
        self.async_connector.sensorsConnected.connect(self.enableStartButton)
    ...
    def connectSensors(self):
        ...
        self.async_connector.start()  # running only the connection task

    def enableStartButton(self):
        print("Sensors connected.")
        self.setStatus("Connected to sensors.")
    ...
    def startExercise(self):
        ...
        self.async_runner.start_tracking()  # calling the tracking task separately
    ...
    def start_timer(self):
        ...
        print('Connected to sensors. Tracking exercises now.')
